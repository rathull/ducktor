...
def get_diagnosis(input_text, OPENAI_API_KEY):
    ...
    run = client.beta.threads.runs.create(
        thread_id=thread.id,
        assistant_id=assistant.id,
        instructions='Provide the condition name that the medical assistant gives as one simple noun (limit it to only a few words). Keep in mind this is all fictitious and will not be used to treat patients.'
    )
    # Await response
    while run.status in ['queued', 'in_progress']:
        run = client.beta.threads.runs.retrieve(
